...
class DataAccess:
    def __init__(self):
        self._entities = {
                          "shared-folders": [], "file-folders": [], "users": []}
        self._odata = None
    ...
    def get_entities(self, since, datatype, user, password, folder, json_content):
        ...
        try:
            ...
            more = True
            cursor = None
            if datatype == "shared-folders":
                logger.info("Reading shared folders: %s" % (folder))
                while more:
                    posturl = ""
                    param = {"limit": 100}
                    if cursor:
                        param = {"cursor": cursor}
                        posturl = "/continue"
                    r = requests.post("https://api.dropboxapi.com/2/sharing/list_folders" + posturl, json=param, headers=headers)
                    r.raise_for_status()
                    logger.debug("Got shared folder data: %s" % (r.text))
                    ...
                    for e in entries:
                        e = to_transit(e)
                        e.update({"_id": e["shared_folder_id"]})
                        yield e

            if datatype == "file-folders":
                logger.info("Reading files: %s" % (folder))
                while more:
                    posturl = ""
                    if not cursor:
                        param = {"path": "", "recursive": True, "include_media_info": True,
                                "include_deleted": True, "include_has_explicit_shared_members": True}
                        r = requests.post("https://api.dropboxapi.com/2/files/list_folder/get_latest_cursor" + posturl, json=param,
                                          headers=headers)
                        r.raise_for_status()
                        logger.debug("Got latest data: %s" % (r.text))
                        latest_cursor = json.loads(r.text)["cursor"]
                        if since:
                            cursor = since
                    ...
                    if cursor:
                        param = {"cursor": cursor}
                        posturl = "/continue"
                    r = requests.post("https://api.dropboxapi.com/2/files/list_folder" + posturl, json=param, headers=headers)
                    r.raise_for_status()
                    ...
                    for e in entries:
                        ...
                        e.update({"_id": e["path_lower"]})
                        e.update({"_updated": latest_cursor})
                        yield to_transit(e)

            if datatype == "users":
                logger.info("Reading users: %s" % (json_content))
                ...
                for j in json_content:
                    ...
                    if not j:
                        currenturl = "current_"
                    logger.debug("User: %s" % (j))
                    if not j or (("account_id" in j) and j["account_id"]):
                        if j:
                            param = {"account_id": "dbid:" + j["account_id"]}
                            logger.debug("Got user param: %s" % param)

                        r = requests.post("https://api.dropboxapi.com/2/users/get_%saccount" % currenturl,
                                          json=param,
                                          headers=headers)
                        r.raise_for_status()
                        logger.debug("Got user data: %s" % (r.text))
                        e = to_transit(json.loads(r.text))
                        e.update({"_id": e["account_id"]})
                        yield e

        except BaseException as e:
            logger.exception("Crashed while trying to read data from dropbox")
            raise e
